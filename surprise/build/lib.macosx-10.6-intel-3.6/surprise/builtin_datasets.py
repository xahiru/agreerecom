...
BUILTIN_DATASETS = {
    ...
    'bookcrossing':
        BuiltinDataset(
            ...
            path=join(get_dataset_dir(), 'bookcrossing/BX-Book-Ratings.csv'),
            rating_scale=(1, 10),
            reader_params=dict(line_format='user item rating',
                               sep=';', skip_lines=1049782, quoted_value='"')
        ),
    'ml-latest-small':
        BuiltinDataset(
    ...
    'jester':
        BuiltinDataset(
            ...
            rating_scale=(-10, 10),
            reader_params=dict(line_format='user item rating', skip_lines=1661440)
        )
}
