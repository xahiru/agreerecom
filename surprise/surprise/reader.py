...
class Reader():
    ...
    def __init__(self, name=None, line_format='user item rating', sep=None,
                 rating_scale=(1, 5), skip_lines=0, quoted_value=None):
    ...
    def parse_line(self, line):
        ...
        # line = (line.decode('utf-8','ignore').encode("utf-8") for i in self.indexes)
        try:
            if self.with_timestamp:
                ...
                uid, iid, r = (line[i].strip()
                               for i in self.indexes)
                timestamp = None
            # iid = iid.decode('utf-8','ignore').encode("utf-8")
            # if self.quoted_value != None:
                # r = r.strip(self.quoted_value)
