...
# param_grid = {'k': [10,20,30,40],'epsilon':[0.01,0.1,0.5,0.9], 'user_based': [False], 'beta':[beta], 'sim_options': {'name': ['pearson'],
                              # 'min_support': [1, 5],
                              # 'user_based': [False]}}
...
# #########
sim_options={'name':'pearson','user_based':False}
# cross_validate(MyOwnAlgorithm(k=40, alog=KNNWithMeans,user_based =False, beta=beta, epsilon=0.9, sim_options=sim_options), data, measures=['RMSE', 'MAE'], n_jobs=-1, cv=5, verbose=True)
# cross_validate(KNNWithMeans(k=40,sim_options=sim_options), data, measures=['RMSE', 'MAE'],n_jobs=-1, cv=5, verbose=True)
cross_validate(OdnovanAlgorithm(alog=KNNWithMeansC, user_based=False, sim_options=sim_options, alpha=0.2), data, measures=['RMSE', 'MAE'], cv=5, verbose=True)
##
# kf = KFold(n_splits=5)
